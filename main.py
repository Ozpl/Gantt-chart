            ...
            if machine.current_product is not None:                
                ...
                #If product is done, try to move it to next machine
                    ...
                    if available_machines.get(machine.output):
                        ...
                        results[str(machine_list[available_machine_index].current_product.sequence)].append(tuple((machine_list[available_machine_index].current_product.starting_point, machine_list[available_machine_index].current_product.starting_point + machine_list[available_machine_index].current_product.goal, machine_list[available_machine_index].name)))
                        machine.current_product = None
        ...
        if to_produce_counter > 0:
            if available_machines.get(first_machine_type):
                for available_machine_index in available_machines[first_machine_type]:
                        ...
                        results[str(top_product.sequence)] = [ tuple((top_product.starting_point, top_product.starting_point + top_product.goal, machine_list[available_machine_index].name)) ]
